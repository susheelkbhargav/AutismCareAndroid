...
public class SoundLevelButton extends MaskedColorView {
    ...
    protected static final int[] STATE_LISTENING = new int[] { ai.kudan.R.attr.state_listening };
    ...
    private SoundLevelCircleDrawable.Params getParams(@NonNull final Context context, @Nullable final AttributeSet attrs) {
        if (attrs != null) {
            final TypedArray viewAttrs = context.obtainStyledAttributes(attrs, ai.kudan.R.styleable.SoundLevelButton);

            try {
                this.listening = viewAttrs.getBoolean(ai.kudan.R.styleable.SoundLevelButton_state_listening, false);

                final float maxRadius = viewAttrs.getDimension(ai.kudan.R.styleable.SoundLevelButton_maxRadius, -1);
                final float minRadius = viewAttrs.getDimension(ai.kudan.R.styleable.SoundLevelButton_minRadius, -1);
                final float circleCenterX = viewAttrs.getDimension(ai.kudan.R.styleable.SoundLevelButton_circleCenterX, -1);
                final float circleCenterY = viewAttrs.getDimension(ai.kudan.R.styleable.SoundLevelButton_circleCenterY, -1);
                final int centerColor = viewAttrs.getColor(ai.kudan.R.styleable.SoundLevelButton_centerColor, SoundLevelCircleDrawable.CENTER_COLOR_DEF);
                final int haloColor = viewAttrs.getColor(ai.kudan.R.styleable.SoundLevelButton_haloColor, SoundLevelCircleDrawable.HALO_COLOR_DEF);
