...
import ai.kudan.android.AIConfiguration;
import ai.kudan.AIListener;
import ai.kudan.android.AIService;
import ai.api.AIServiceException;
import ai.kudan.PartialResultsListener;
import ai.api.RequestExtras;
import ai.api.model.AIError;
import ai.api.model.AIRequest;
import ai.api.model.AIResponse;
import ai.kudan.services.GoogleRecognitionServiceImpl;
...
public class AIButton extends SoundLevelButton implements AIListener {
    ...
    protected static final int[] STATE_WAITING = {ai.kudan.R.attr.state_waiting};
    protected static final int[] STATE_SPEAKING = {ai.kudan.R.attr.state_speaking};
    protected static final int[] STATE_INITIALIZING_TTS = {ai.kudan.R.attr.state_initializing_tts};
    ...
    public enum MicState {
        ...
        public static MicState fromAttrs(final TypedArray viewAttrs) {
            if (viewAttrs.getBoolean(ai.kudan.R.styleable.SoundLevelButton_state_listening, false))
                return listening;
            if (viewAttrs.getBoolean(ai.kudan.R.styleable.SoundLevelButton_state_waiting, false))
                return busy;
            if (viewAttrs.getBoolean(ai.kudan.R.styleable.SoundLevelButton_state_speaking, false))
                return speaking;
            if (viewAttrs.getBoolean(ai.kudan.R.styleable.SoundLevelButton_state_initializing_tts, false))
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (animationStage > 0 || animationSecondPhase) {
            ...
            paint.setColor(getResources().getColor(ai.kudan.R.color.icon_orange_color));
