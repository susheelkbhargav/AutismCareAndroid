...
import ai.kudan.util.VoiceActivityDetector;
...
public class SpeaktoitRecognitionServiceImpl extends AIService implements
    ...
    private class RecorderStream extends InputStream {
        ...
        public int read(@NonNull final byte[] buffer, final int byteOffset, final int byteCount) throws IOException {
            ...
            if (bytesRead > 0) {
                synchronized (bytesLock) {
                    ...
                    while (bytes.length >= VoiceActivityDetector.FRAME_SIZE_IN_BYTES) {
                        final byte[] b = new byte[VoiceActivityDetector.FRAME_SIZE_IN_BYTES];
                        System.arraycopy(bytes, 0, b, 0, VoiceActivityDetector.FRAME_SIZE_IN_BYTES);
                        vad.processBuffer(b, VoiceActivityDetector.FRAME_SIZE_IN_BYTES);

                        temp = bytes;
                        final int newLength = temp.length - VoiceActivityDetector.FRAME_SIZE_IN_BYTES;
                        bytes = new byte[newLength];
                        System.arraycopy(temp, VoiceActivityDetector.FRAME_SIZE_IN_BYTES, bytes, 0, newLength);
                    }
        ...
        private void normalize(@NonNull final byte[] buffer, final int bytesRead) {
            final int remainOffset = VoiceActivityDetector.NOISE_BYTES - offset;
